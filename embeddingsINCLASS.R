...
setwd("C:/Users/16108/Downloads/TAD_2021-master(1)/TAD_2021-master/R lessons")
...
library(quanteda.textstats)
author <-  word_vectors["maserati", , drop = FALSE] - 
  word_vectors["bugatti", , drop = FALSE] +
  word_vectors["lamborghini", , drop = FALSE]
cos_sim <- textstat_simil(as.dfm(word_vectors), y = as.dfm(author),
                          margin = "documents", method = "cosine")
head(sort(cos_sim[, 1], decreasing = TRUE), 5)
#above code returns the closest vectors for maserati-bugatti + lamborghini,
#which are ebb, savants, definitional, caesarapapism, and ibex
