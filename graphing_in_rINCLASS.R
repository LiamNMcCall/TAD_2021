...
library(quanteda)
ahabDFM <- dfm(data_char_mobydick, remove = stopwords("english"), remove_punct = TRUE)
ahabfeatures <- topfeatures(ahabDFM, 100)
topDf <- data.frame(
  list(
    term = names(ahabfeatures),
    frequency = unname(ahabfeatures)
  )
)
topDf$term <- with(topDf, reorder(term,-frequency))
topDf$rank <- seq(1:100)
library(ggplot2)
ggplot(topDf, aes(x=rank, y=frequency)) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
geom_point(color = "blue")
